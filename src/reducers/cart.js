// const cartReducer = (state = [], action) => {
//     let newState=[...state]
//     switch (action.type) {
//         case "ADD_TO_CARD":
//             return [
//                 ...state,
//                 {
//                     id: action.id,
//                     info: action.info,
//                     quantity: 1
//                 }
//             ]
//         case "UPDATE_QUANTITY":
//             const itemUpdate=newState.find(item => item.id === action.id)
//             itemUpdate.quantity=itemUpdate.quantity + action.quantity; 
//             return newState
//         case "DELETE_ITEM":
//             newState = newState.filter(item => item.id !== action.id)
//             return newState
//         case "DELETE_ITEM_ALL":
//             return [];
//         default:
//             return state
//     }
// }
// export default cartReducer;
const cartReducer = (state = [], action) => {
    let newState = [...state]; // Sử dụng shallow copy của state
    switch (action.type) {
        case "ADD_TO_CART":
            return [
                ...state,
                {
                    id: action.id,
                    info: action.info,
                    quantity: 1, // Mặc định số lượng là 1 khi thêm sản phẩm mới
                }
            ];
        case "UPDATE_QUANTITY":
            const itemUpdate = newState.find(item => item.id === action.id);
            if (itemUpdate) {
                itemUpdate.quantity += action.quantity; // Cập nhật số lượng
            }
            return newState; // Trả về giỏ hàng đã được cập nhật
        case "DELETE_ITEM":
            newState = newState.filter(item => item.id !== action.id);
            return newState; // Xóa sản phẩm khỏi giỏ hàng
        case "DELETE_ITEM_ALL":
            return []; // Xóa toàn bộ giỏ hàng
        default:
            return state;
    }
};

export default cartReducer;
